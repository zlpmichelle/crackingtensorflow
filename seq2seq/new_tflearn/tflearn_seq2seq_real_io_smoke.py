...
#<NEEDS UPDATING> exists in 4 places which should be updated

run example: python tflearn_seq2seq_real.py --in-len=100 -e 2 predict --from-file=seq_test.txt 
'''
...
class TFLearnSeq2Seq(object):
    ...
    def generate_trainig_data(self):
        ...
        with label as file:
            for line in file:
                line = line.split(' ')
                y_data.append([int(i) for i in line])

        x_data = np.reshape(x_data, (lines, self.in_seq_len))
        y_data = np.reshape(y_data, (lines, self.in_seq_len))
    ...
    def sequence_loss(self, y_pred, y_true):
        ...
        sl = tf.contrib.legacy_seq2seq.sequence_loss(logits, targets, weights)
    ...
    def train(self, num_epochs=20, model=None, model_params=None, weights_input_fn=None,
        ...
        print ("[TFLearnSeq2Seq] Training on dataset (pattern '%s'), with %d epochs" % (self.sequence_pattern.PATTERN_NAME, num_epochs))
        # tf.reset_default_graph()
        if self.verbose > 1:
...
class SequencePattern(object):
    ...
    def generate_output_sequence(self, x):
        ...
        '''
        #<NEEDS UPDATING>
        return [int(i) for i in self.label.readline().split(' ')]
...
"""
    ...
    if args.cmd=="predict":
        if args.from_file:
            testFile = open(args.from_file)
            test_data = []
            with testFile as file:
                for line in file:
                    line = line.split(' ')
                    test_data.append([int(i) for i in line])
            inputs = test_data

            #inputs = json.loads(args.from_file)
            ##### INPUT AND FORMAT DATA HERE
        ...
        model = ts2s.train(num_epochs=args.epochs, weights_output_fn=args.output_weights,
        ...
        if not args.from_file:
            try:
                ...
                raise Exception("Please provide a comma-delimited input sequence as the argument")

        sp = SequencePattern(args.pattern_name, in_seq_len=args.in_len, out_seq_len=args.out_len)
        ts2s = TFLearnSeq2Seq(sp, seq2seq_model=args.model, data_dir=args.data_dir, name=args.name, verbose=args.verbose)
        results = []
        #<NEEDS UPDATING>
        fwrite = open("test_res_prediction.txt", 'w')
        for x in inputs:
            prediction, y = ts2s.predict(x, model=model, weights_input_fn=args.input_weights, model_params=model_params)
            #print("==> For input %s\n, prediction=%s\n (expected=%s)" % (x, prediction, sp.generate_output_sequence(x)))
            fwrite.write(np.array2string(prediction) + "\n")
